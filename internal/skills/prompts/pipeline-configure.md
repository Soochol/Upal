...
Research fields:
- `research_depth`: Default research depth. `"light"` = single search pass (fast), `"deep"` = iterative agent loop with sub-question decomposition (thorough). Default `"deep"`.
...
8. **Research model**: When setting `research_model` or research source `model`, ONLY use models that support native `web_search` tool. Do NOT use Ollama models for research.
