...
import { computeLayout, edgePath, dominantCssVar } from '@/shared/lib/workflowSvg'
import type { LayoutNode, LayoutEdge } from '@/shared/lib/workflowSvg'
...
const CSS_VARS: Record<string, string> = Object.fromEntries(
  getAllNodeDefinitions().map((def) => [def.type, def.cssVar]),
)
...
export function WorkflowMiniGraph({ nodes, edges, uid }: Props) {
  ...
  const nodeDefs = getAllNodeDefinitions()
  ...
  return (
    ...
    >
      ...
      {nodes.map((n) => {
        ...
        if (!pos) return null
        const cssVar = nodeDefs.find((d) => d.type === n.type)?.cssVar ?? 'var(--muted-foreground)'
        return (
          <g key={n.id}>
            {/* Soft halo */}
