                ...
                <div className={[
                  ...
                  <div className="px-3 py-3">
                    ...
                    {!hasOutput && !result?.error && result && !['pending', 'running'].includes(stageStatus) && (
                      <p className="text-[11px] text-muted-foreground/40 italic">No output</p>
                    )}

                    {/* Pending placeholder */}
                    {stageStatus === 'pending' && (
                      <p className="text-[11px] text-muted-foreground/40">Waiting</p>
                    )}

                    {/* Approve / Reject */}
                    {isWaiting && (
                      <div className={`flex items-center gap-2 ${(hasOutput || result?.error) ? 'mt-3 pt-3 border-t border-border' : ''}`}>
                        <span className="text-xs text-muted-foreground flex-1">Awaiting approval</span>
                        <button
