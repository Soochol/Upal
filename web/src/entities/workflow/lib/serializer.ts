...
export function deserializeWorkflow(
  ...
  const regularNodes: Node<NodeData>[] = wf.nodes.map((n, i) => {
    ...
    // Get description from registry, with fallback for unknown types
    let nodeDescription = ''
    try {
      nodeDescription = getNodeDefinition(n.type as NodeType).description
    } catch {
      nodeDescription = ''
    }
    ...
    const node: Node<NodeData> = {
      ...
      data: {
        ...
        description: (n.config.description as string) || nodeDescription,
        config: n.config,
      },
    }
