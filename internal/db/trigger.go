...
func (d *DB) CreateTrigger(ctx context.Context, t *upal.Trigger) error {
	...
	_, err := d.Pool.ExecContext(ctx,
		`INSERT INTO triggers (id, workflow_name, type, config, enabled, created_at)
		 VALUES ($1, $2, $3, $4, $5, $6)`,
		t.ID, t.WorkflowName, string(t.Type), configJSON, t.Enabled, t.CreatedAt,
	)
	if err != nil {
...
func (d *DB) GetTrigger(ctx context.Context, id string) (*upal.Trigger, error) {
	...
	err := d.Pool.QueryRowContext(ctx,
		`SELECT id, workflow_name, type, config, enabled, created_at
		 FROM triggers WHERE id = $1`, id,
	).Scan(&t.ID, &t.WorkflowName, &triggerType, &configJSON, &t.Enabled, &t.CreatedAt)
...
func (d *DB) ListTriggersByWorkflow(ctx context.Context, workflowName string) ([]*upal.Trigger, error) {
	...
	for rows.Next() {
		...
		if err := rows.Scan(&t.ID, &t.WorkflowName, &triggerType, &configJSON, &t.Enabled, &t.CreatedAt); err != nil {
